...
class PhaseTruthVerifier:
    ...
    def check_session_log(self) -> Tuple[bool, float, str]:
        ...
        Phase 3+: A/B removed — skip this check (Phase End Audit替代).
        """
        if self.phase > 2:
            raise InfraSkip("A/B removed for Phase 3+ (Phase End Audit替代)")
    ...
    def check_ab_coverage(self) -> Tuple[bool, float, str]:
        """HR-01: Each fr_id in sessions_spawn.log must have both developer and reviewer.

        Detects missing Agent B review for specific deliverables (e.g., TEST_PLAN.md
        generated by Agent A but never reviewed by Agent B/Architect).
        ...
        if not log_path.exists():
            # Fallback: root path (pre-migration repos)
            log_path = self.project_root / "sessions_spawn.log"
        if not log_path.exists():
            return False, 0.0, "sessions_spawn.log absent — cannot verify A/B pairing"

        try:
            content = log_path.read_text(encoding="utf-8", errors="replace")
        except Exception as e:
            return False, 0.0, f"Cannot read sessions_spawn.log: {e}"

        # Accepted reviewer role names (HR-01 allows various role names)
        _REVIEWER_ROLES = {"reviewer", "architect", "tech_lead", "qa_lead", "senior_dev"}

        fr_roles: dict[str, set[str]] = {}
        for line in content.splitlines():
            ...
            try:
                ...
                continue
            fr_id = str(entry.get("fr_id", "")).strip()
            role = str(entry.get("role", "")).strip().lower()
            if fr_id and role:
                fr_roles.setdefault(fr_id, set()).add(role)

        if not fr_roles:
            return False, 0.0, "No valid entries in sessions_spawn.log"

        missing_reviewer = [
            fr for fr, roles in fr_roles.items()
            if not roles.intersection(_REVIEWER_ROLES)
        ]

        total = len(fr_roles)
        missing_count = len(missing_reviewer)

        if missing_count == 0:
            return True, 100.0, f"All {total} deliverable(s) have A/B pairing ✓"

        score = max(0.0, 100.0 * (total - missing_count) / total)
        top_missing = sorted(missing_reviewer)[:5]
        return False, score, (
            f"{missing_count}/{total} deliverable(s) missing reviewer: "
            f"{', '.join(top_missing)}"
