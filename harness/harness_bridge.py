...
class EnvCheckContext:
    ...
    def evaluation_prompt(self) -> str:
        ...
        return (
            ...
            f"     - Not exported, NO documented default?   → required.present: false\n"
            f"   \n"
            f"   Example: SPEC §5.1 declares 8 TASKQ_* vars each with a default\n"
            f"   value. If none are exported: put all 8 names in optional_missing,\n"
            f"   leave env_vars.required as an empty array, and set ready=true —\n"
            f"   the project IS ready with zero exported env vars.\n\n"
            f"[FORBIDDEN]\n"
