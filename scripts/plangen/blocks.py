...
def _fr_dev_steps(fr_id: str, phase: int, project: Path) -> List[str]:
    ...
    return [
        ...
        f"  python3 harness_cli.py review-fr-tests --fr-id {fr_id} --phase {phase} --project .",
        "  ```",
        f"  → review-fr-tests REJECT: fix the TEST so it fails when SPEC.md's {fr_id} requirement",
