...
# Artifact parsers moved to scripts/plangen/artifact_parsers.py (Round 3 M2).
# Package-qualified import on purpose: this module already requires the
# harness root on sys.path (the `from core...` imports above say so loudly),
# and under that precondition `scripts.plangen` resolves for BOTH module
# identities — `scripts.generate_full_plan` and bare `generate_full_plan`
# (tests put the repo root on sys.path alongside scripts/).
...
# Pure re-exports (not referenced below, but part of this module's public
# surface — tests and cli/ import them from here; same convention as
# cli/_shared.py's gate1_evidence re-export block).
from scripts.plangen.artifact_parsers import (
...
# Shared block builders moved to scripts/plangen/blocks.py (Round 3 M3;
# same package-qualified import rationale as the M2 block above).
...
# Pure re-exports (public surface of this module — tests import them
# from here; cli/_shared.py convention).
from scripts.plangen.blocks import (
