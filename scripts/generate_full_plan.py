...
_PHASE_DELIVERABLE_DEPS: Dict[int, List[Dict]] = {
    1: [
        {
            ...
            "depends_on": [],
            "task_hint": "Check PROJECT_BRIEF.md. If canonical spec exists: Switch to INGESTION MODE (DO NOT invent, 100% transcribe endpoints/constraints). If none: Elicit requirements → write FRs/NFRs in SRS.md (### FR-XX: format)",
            "checks": ["Did Agent A successfully transcribe ALL features from the canonical spec (if one exists) into SRS.md, or leave it empty?",
    ...
    2: [
        ...
        },
        {
            ...
            "depends_on": ["ADR.md"],
            "task_hint": "Generate TEST_SPEC.md via derive_test_cases.md skill → preserve TEST_INVENTORY.yaml names where specified → apply 7-Question Protocol per FR → fill concrete Inputs + a Sub-assertion predicate table per FR → run check-test-spec-consistency → populate cross-cutting section",
