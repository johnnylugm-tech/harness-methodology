...
def _review_checkpoint(phase: int, checkpoint_n: int) -> List[str]:
    ...
    lines += [
        ...
        "    > Human fix → re-dispatch Agent B (same prompt + updated content) → `APPROVE` required before continuing.",
        "",
        f"- **[B-APPROVAL]** ✅ Persist Agent B approval JSONs for each deliverable to `.methodology/agent_b_approvals/<id>.json`",
        ...
        f"  > Phase {phase} deliverable IDs = phase deliverables (see `harness_cli.py _PHASE_DELIVERABLES[{phase}]`). For Phase 1: SRS, SPEC_TRACKING, TRACEABILITY_MATRIX, TEST_INVENTORY.",
