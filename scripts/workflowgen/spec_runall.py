...
def _render_driver(all_titles: list[str]) -> str:
    return (
        ...
        "phase: n, phases_run: phasesRun, detail: outcome, "
        "note: 'PHANTOM direction (exit 45): SAB.json declares a module the codebase does not implement. "
        "Resolve each phantom with `harness_cli.py amend-sab --resolve-phantom <declared> --to <target>|--drop --reason \">=20 chars\"`, "
        "then re-run with a NEW run_tag: Workflow({scriptPath, args: {repo, run_tag}}). amend-sab changes no prompt, "
        "so without one the cache can replay this halt.' })\n"
        "  }\n"
        ...
        "phase: n, phases_run: phasesRun, detail: outcome, "
        "note: 'INFRA direction (exit 25, UNREGISTERED): code→SAB drift or a tool that never ran. "
        "Repair project state with `harness_cli.py amend-sab`, then re-run with a NEW run_tag: "
        "Workflow({scriptPath, args: {repo, run_tag}}). amend-sab changes no prompt, "
        "so without one the cache can replay this halt.' })\n"
        "  }\n"
        ...
        "    return halt('repeated-failure', { error: 'run-all stopped in Phase ' + n + ': ' + String(outcome.message || 'repeated-failure refusal (exit 36)'), "
        "phase: n, phases_run: phasesRun, detail: outcome, "
        "note: 'exit 36 — run-fr-step refuses to re-dispatch an identical failure already seen on this tree. "
        "Read the consuming project\\'s .methodology/degradations.jsonl for the signature; any change to the tree "
        "re-opens the step, then re-run.' })\n"
        "  }\n"
