...
//
// Structure: NO FR loop. Gate 4 ({S.gate_dim_count(4)} dims, tool-scored + artifact-backed DA
...
def generate_phase6() -> str:
    parts = [
        ...
        _render_meta(
            name="phase6-quality",
            description=(
                f"Phase 6 Quality — Gate 4 ({S.gate_dim_count(4)} dims + DA challenge) + Agent B peer review "
        ...
        B.render_gate_loop(
            gate_num=4, phase=6,
            log_msg=f"Gate 4 full-project eval ({S.render_gate_dims_summary(4)})",
            prompt_steps=_GATE4_STEPS,
            pass_line_desc=S.render_gate_pass_line(
                4, d4_threshold=_D4_THRESHOLD_P6,
                extra=" AND DA artifacts present"),
