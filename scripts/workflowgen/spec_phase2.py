...
def _render_phase2_sab_generation() -> str:
    return (
        ...
        + "  + '   - nfr_traceability: one entry per NFR enumerated from SPEC.md (parse `### NFR-XX:` headings — do not assume a fixed NFR count) with a `type` from the legal values (" + _NFR_TYPES + ") + measurable `target` + `module`. CRITICAL: `type:` and `dimension:` are TWO DIFFERENT FIELDS — the type vocabulary above is 14 names; the dimension vocabulary below is a separate 18+-name list. A name ONLY in the dimension list is NOT legal in `type:` and will be REFUSED. Copy the SPEC.md `dimension:` value into `dimension:`, NOT `type:`. For dimension-only NFRs (e.g. error_handling, architecture_constraints), pick the nearest type (reliability, layering).\\n'\n"
