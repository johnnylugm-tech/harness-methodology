...
| `gate_evidence_max_bytes` | `1048576` | Round 45 站1 per-file ceiling for copying a dimension's cited `tool_output` into `.methodology/gate_evidence/gate{N}/` at finalize time, so the verdict's citations survive the gitignored `.sessi-work/`. Over the ceiling the citation is left pointing at the original and a `gate:evidence-too-large` row lands in the degradation ledger. Sized from measurement: the largest cited evidence across five projects was 19,994 bytes. |
