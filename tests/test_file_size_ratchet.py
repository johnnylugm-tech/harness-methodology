...
_LINE_CEILING: dict[str, int] = {
    ...
    # after the phase entry that verified it.
    "cli/fr_cmds.py": 2283,
    ...
    "scripts/workflowgen/js_blocks.py": 1699,  # 2026-08-13: +29 — Round 50 站3: HALT_FN_BLOCK, the hoisted `halt(step, shape)` every terminal exit in a generated workflow now returns through. Measured before writing it: the eight phase files reach run-all's loop from 55 distinct top-level `return { error: ... }` sites and every one is recorded under the single step name `phase-error`, so a full P1-P8 run produced one workflow_blocks.jsonl row naming a phase and nothing else. All of the growth is the block's own comment recording that measurement and why the helper costs no dispatch (the driver already records at its boundary; only the step name was missing). The rewritten call sites are net-neutral to slightly negative here — they gained `halt('slug', ` and lost nothing. Previous: 1670.  # 2026-08-07: +55 — Round 43 站3: render_sync_verified gained a bounded retry WITH repair authority, a [HARNESS-BUG] early exit, and an `on_blocked` terminal branch so P3's bespoke second Sync implementation could be deleted (spec_phase3.py:_render_phase3_sync went from ~60 rendered lines to a call plus its own terminal). Net across the two files is roughly flat; the growth here is the SYNC_REPAIR_CLAUSE constant (tests/test_sync_may_repair.py asserts it appears in every shipped Sync block, so it may not be inline prose) and the comment recording why a bare re-send at a pre-push content blocker was the defect Round 41 站3 named. Splitting js_blocks.py is a separate question from this change and is not attempted here.
}
