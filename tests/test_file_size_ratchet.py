...
_LINE_CEILING: dict[str, int] = {
    ...
    "cli/fr_cmds.py": 2366,  # 2026-08-23: +1 — code-review follow-up: `_abort_no_progress_with_self_doubt` gained a `phase: int` parameter (was printing the step-name string into `--phase`, which argparse rejects as a non-int) — see `_detect_evaluator_passed_but_commit_uncommitted`'s sibling fix below for the score_gate correction in the same neighborhood. Previous: 2365.
