...
_LINE_CEILING: dict[str, int] = {
    ...
    "cli/fr_cmds.py": 2728,  # 2026-07-21: +30 lines — fix/fr-step-already-done-cascade follow-up: cascade code is the same +30 as the +42 above, but the final ceiling absorbed two follow-up adjustments: (i) restrict the GATE1/GATE1-DELTA early-return to `phase is not None` (was: unconditional, broke the `test_gate1_already_done_uses_quality_complete_not_overall_score` regression test for `phase=None` callers), (ii) split the unknown-step fallback from `return True` (pre-fix dead code) into explicit `return True` for TDD-IMPROVE / AMEND-SAB / GATE1 when commit-grep succeeded, plus a conservative `return False` for MIRROR / ORCH-POST (PR #18 added AMEND-SAB to `_FR_STEP_COMMIT_PATTERNS`, exposing the previously-unreachable fallback line for the first time).
