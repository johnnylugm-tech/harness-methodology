...
_LINE_CEILING: dict[str, int] = {
    ...
    "cli/fr_cmds.py": 2615,  # 2026-07-21: +15 lines — fix/sab-sync-improve-to-gate1: deterministic `amend-sab` dispatch branch (`run-fr-step --step amend-sab` delegates to `cmd_amend_sab` without spawning an LLM; SSOT-bridged from `agent_spawner._COMMIT_REQUIRED_STEPS` into the argparse `choices`) + extended comment block.
