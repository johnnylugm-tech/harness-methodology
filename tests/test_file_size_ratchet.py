...
_LINE_CEILING: dict[str, int] = {
    ...
    "cli/fr_cmds.py": 2750,  # 2026-07-21: +18 lines — fix/spec-cov-pct-section-boundary-and-dedup: `_extract_test_spec_names()` now reuses `spec_coverage._parse_test_spec()` (the parser `finalize-gate`'s S4 spec-coverage check already uses) instead of a second, independently-drifted line-by-line parser whose section-boundary regex only reset `current_fr` on `### FR-XX` headings — any other heading (e.g. `### NFR Integration (...)`) left `current_fr` stuck, leaking later sections' test names into the FR's denominator (FR-05 repro: 11 real rows inflated to 16). `_compute_fr_spec_data()` also gained a `covered_row_count` counter computed row-for-row against `spec_test_names` (mirrors `spec_coverage._run_spec_coverage_check()`'s list-based `covered = [i for i in items if ...]`) instead of dividing a de-duplicated `set`-based numerator by a non-deduplicated `list`-based denominator — TEST_SPEC.md's v2.13.0 "Multi-scenario expansion" rule deliberately repeats one function name across N parametrize rows, and the old asymmetric dedup manufactured N-1 permanently-unsatisfiable "missing" entries regardless of actual coverage (FR-05 repro: `spec_cov_pct` wrongly capped at 75% instead of the true 100%, causing GATE1 to falsely BLOCK for 7 consecutive rounds before a sub-agent got lucky and overrode the bad prompt hint).
