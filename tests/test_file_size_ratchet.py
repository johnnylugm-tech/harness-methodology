...
_LINE_CEILING: dict[str, int] = {
    ...
    "scripts/workflowgen/js_blocks.py": 2315,  # 2026-09-07: +67 — Round 102 站3: render_terminal_abort_detectors gains the park_rcs option (park blocks for 25/36) and the frRc===36 return branch for delta hosts, plus the _park_block helper and the delta prompt STOP-list update. Previous: 2248.  # 2026-09-06: +21 — Round 100 站1 (PHANTOM exit code split). The new `frRc===45` detector in `render_terminal_abort_detectors` returns `{phantom_abort: true, condition_class: 'PHANTOM', ...}` symmetric to the existing 25 branch; inlined at 6 per-FR sites in run-all.js plus their standalone phase3-8 copies. Docstring at lines 700-714 updated from "three classes, three codes" to "four classes, four codes since Round 100 站1". A minor tweak to the comment avoided mentioning `amend-sab` by name (which would have inflated `tests/test_workflowgen.py::TestOrchPostIsPerPhaseNotPerFr`'s amend-sab count). Previous: 2227.
