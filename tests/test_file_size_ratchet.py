...
_LINE_CEILING: dict[str, int] = {
    ...
    "cli/fr_cmds.py": 2242,  # 2026-09-08: +17 — Round 109 站6: COVERAGE-FIX's false-positive test can no longer be made against a floor the project never declared. `min_coverage_floor` returns None for an absent `quality_targets.min_coverage` instead of the framework's own 80.0, so the `_live_cov >= _cov_min` comparison is guarded and an undeclared floor falls through to the operator break with a ledger row naming the missing key. This site has no behavioural driver in the suite (the branch is inside cmd_run_fr_step, which nothing here calls) and the test file says so rather than implying coverage it does not have. Previous: 2225.  # 2026-09-07: +53 — Round 102 站2 (canonical failure-memory rows): _note_turn_budget_kill / _note_wallclock_kill write step-failure rows with empty-output signatures on both branches, the fix-round dispatch gains a repeated_failure pre-check, and the two blocked-human exits (post-loop + _abort_no_progress_with_self_doubt) record canonical GATE1_BLOCKED rows. Previous: 2172.  # 2026-09-06: +50 — Round 100 站1 (PHANTOM exit code split). `_classify_infra_or_harness_bug` direction-specific-first classifier (was: collapse-all to "INFRA"; now: PHANTOM / UNREGISTERED / INFRA / HARNESS_BUG). `_abort_dispatch_infra_or_harness_bug` 3-way return (was: HARNESS_BUG or 25; now: 70 or 45 or 25) with the new PHANTOM/UNREGISTERED print blocks naming each remediation channel. Previous: 2122.
    ...
    "core/quality_gate/spec_coverage.py": 956,  # 2026-09-09: +13 — Round 111 站F1. 943 -> 956. Four lines are the call and the print; the rest is the comment recording why the file the P1 Naming Authority check reads is not the declaration it was written to protect. That check compares the WORKING-TREE TEST_INVENTORY.yaml against TEST_SPEC.md, so retracting a declared name satisfies it — and `state.json.test_inventory_checksum`, written at the P1 exit since v2.6.1 for exactly this comparison, had one writer and zero readers. 15 of the 21 corpus projects carry a digest that no longer matches their file. The judgement is deliberately NOT made here: the movement is printed and recorded (core/quality_gate/naming_authority.py), because taskq-new's 50 retracted names include `test_fr10_ac5_status_mapping_422_401_403_404_409_429_503_500` split into the eight TEST_SPEC.md rows it always meant. Previous: 943.  # 2026-09-06: +113 — new god file, Round 99 站1/站2. Previous: 830 (unlisted). Crossing the 900 threshold on the day it is granted, so the decision is reviewed here rather than drifted into. Three additions, all of them the file's own subject (what a TEST_SPEC declaration row says, and what the framework does when it cannot tell). 站1 is `_cell_identifier` (+42): the identifier normalisation was one expression whose two steps were order-dependent in a way nothing declared, and a cell written `` `test_x` [AC-1.1] `` — the shape the framework's own Step 1d asks for and its five-column template has no room for — kept a stranded backtick and was dropped. Measured on taskq-done: 109 of 120 rows gone, `declared=11`, and the P1 Naming Authority check reporting 91 of 91 inventory names "missing in TEST_SPEC.md — Agent A may have hallucinated names" about names that were present. About 30 of those lines carry that measurement. 站2 is `unreadable_declarations` (+24) and the block that reads it (+47): the population `_report_unread_rows` has recorded since Round 74 splits in two — a row stating it declares nothing, which MEASUREMENT_SINKS.yaml defends and must stay report-only, and a row declaring a test nobody can read, which until now was scored identically to a row that does not exist. Splitting this file was considered and rejected for this round: every line added is the parse or the verdict over the parse, and a split in the same commit as a behaviour change is the one thing tests/test_god_file_split_safety.py exists to stop. Previous: unlisted (830).
    "core/quality_gate/import_contracts.py": 308,  # 2026-09-08: +308 — Round 106 站C, a new file that is not new code. Previous: 0. 272 of these lines moved byte-identically out of arch_constraints.py (which drops 950 → 687 in the same commit) plus a docstring recording why this half is the half that moves. First ceiling is its measured length.
    "core/quality_gate/arch_constraints.py": 686,  # 2026-09-08: -264 — Round 106 站C cut the leaf group out to core/quality_gate/import_contracts.py (`_config_sources` / `layer_module_tails` / `_contract_sources` / `contract_decides` / `read_import_contracts` / `read_bandit_config`, plus `_LAYER_GRAMMAR` and `_MODULE_WILDCARD`). A harvest, not a raise: the ceiling follows the file down, so the space cannot be silently re-filled. The six bodies were fingerprinted BEFORE the move (Round 106 站C-網) and every fingerprint is unchanged — that is the proof this was a move. `arch_constraints` re-exports all eight names, so no import outside changed. Round 105 deferred this split with the re-open condition "下一次它再長"; the file did not grow, the round that could weave the net first arrived. Previous: 950.  # 2026-09-07: +107 — new god file, Round 105 站1a. Previous: 843 (unlisted). Crossing the 900-line threshold is reviewed here rather than drifted into, and what crossed it is mostly comment: `layer_module_tails` is nine lines of code under a docstring that records why the grammar may not live in this repository, and `_contract_sources` is twenty-five under one that records what an empty list would mean if it were returned instead of None. The defect being fixed is that this file split `layers = ` on newlines and stopped, so import-linter's `|` (independent siblings), `:` (non-independent), `(m)` (optional) and `containers` (a prefix, not a name) were read as characters in a module name. taskq-sn wrote `taskq_api.config | taskq_api.exceptions`, contract_coverage_gap reported both modules outside every contract, Gate 1 blocked, and the project changed its architecture to fit the parser (f97e5be). Measured across the thirteen corpus configs the new parse is byte-identical — zero source sets change — so the growth buys the case the victim had to remove and the ones nobody has hit yet, not a number that moves today. A split is Round 49's 織網-then-cut and is a round of its own; recorded in docs/PROPOSAL_ADJUDICATIONS.md.
    ...
    "scripts/workflowgen/js_blocks.py": 2323,  # 2026-09-08: +8 — abLoop A/B loops guard against agent() terminal-failure null/empty string (reusing sibling halt_step with 'no result (terminal API failure)'). Previous: 2315.  # 2026-09-07: +67 — Round 102 站3: render_terminal_abort_detectors gains the park_rcs option (park blocks for 25/36) and the frRc===36 return branch for delta hosts, plus the _park_block helper and the delta prompt STOP-list update. Previous: 2248.  # 2026-09-06: +21 — Round 100 站1 (PHANTOM exit code split). The new `frRc===45` detector in `render_terminal_abort_detectors` returns `{phantom_abort: true, condition_class: 'PHANTOM', ...}` symmetric to the existing 25 branch; inlined at 6 per-FR sites in run-all.js plus their standalone phase3-8 copies. Docstring at lines 700-714 updated from "three classes, three codes" to "four classes, four codes since Round 100 站1". A minor tweak to the comment avoided mentioning `amend-sab` by name (which would have inflated `tests/test_workflowgen.py::TestOrchPostIsPerPhaseNotPerFr`'s amend-sab count). Previous: 2227.
    ...
    "cli/advance_prechecks.py": 1199,  # 2026-09-09: +30 — Round 111 站F1, at the site that already owns the P1 baseline: eleven lines are the phase guard, the call and the `record_degradation`, and the rest is the comment. The row is filed HERE rather than in spec_coverage because `_run_spec_coverage_check` runs several times per phase (the CLI check, spec_tracking_checker, each per-FR pass) and a ledger row per call is a ledger nobody reads. The trigger is retraction only, and that is a mechanism argument rather than a corpus one: every name in today's file is already compared against TEST_SPEC.md by the live check, so the only thing a frozen copy can add is a name that LEFT. Measured 2026-09-09 across the 21 corpus projects: 11 have moved, 10 purely by addition (P2 derives test cases and the inventory grows — the normal path), and taskq-new alone retracts 50. Triggering on movement in either direction would have filed a row on ten projects doing nothing wrong. Previous: 1169.  # 2026-09-08: +48 — Round 106 站A: `_precheck_framework_examples_were_replaced`. 1121 -> 1169. Fifteen lines are the call, the printed block and the try/except; the rest is the docstring recording why only TWO questions are asked at the delivered end and which two were refused. Round 105 marked the framework's example values and checked only the TEMPLATE end, on a measurement that said marking was the only variable — that measurement grepped `app.api.routes` and the template emits `app.api.webhooks`, so it read 0/13 where the truth is 1/13: taskq-forever ships all four SAB module paths with the marker line still above them. The refused rules are named in the docstring because they are the ones a later reader will reach for: "the marker is still there" charges taskq-renew, which kept the comment and replaced the modules, and "the value is still 80" charges any project that considered the question and chose 80. The decision itself is core/quality_gate/legal_artifacts.framework_examples_in. Measured: 3 of 13 corpus projects report non-empty at one or both phases, and taskq-renew reports zero. Previous: 1121.  # 2026-09-07: +59 — Round 105 站1b: `_precheck_declared_constraints_are_configured`. 1062 -> 1121. Four of the lines are the phase guard and the SAB read; the rest is the call to Round 51/54's existing `classify_constraints` + `unconfigured_blocking_reason`, the printed remedy, and the docstring recording the measurement that justifies a NEW BLOCK at a phase boundary: taskq-sn's SAB at its Phase 2 completion commit (a6bf87f) declares `no_circular_dependencies`, the tree carries no `.importlinter`, and `.importlinter` first appears in 368f9db — a Phase 3 "address Gate1 failing dims" commit. Replayed against that exact tree the check now returns 48 with the config file and section named. No new judgement is added here; the only new thing is that somebody asks at the one phase boundary with no gate behind it. Five of thirteen corpus projects are in the blocking state today (api / new / plus / renew / super), which is the cost and is stated rather than discovered later. Previous: 1062.  # 2026-09-06: +83 — Round 101 站3/站4. 979 -> 1062. Two P3-exit wires and one new function. 站3 is `_precheck_sab_placements_are_declared` (+41 with its docstring): `.methodology/SAB.json` is what `scripts/generate_sab.py` renders from SAD.md §5 — `sab_amender`'s own header calls SAD.md authoritative — and `amend_sab` writes the rendered file directly, which is the rule this repository enforces on its own generated workflow JS, inverted. Measured over the seventeen corpus projects: 117 placements exist only in SAB.json and 42 of those are not even derivable from the project's own layer names. It is a separate function, and it is called from `_precheck_sab_consistency` BEFORE the DriftDetector, because 7 of taskq-done's 11 CRITICAL findings are consequences of two of those placements and reporting them first sends the project to fix an import its own SAD.md never forbade. 站4 is +25 at the existing Round 99 manifest block: `env_contract.json`'s declared toolchain and the delivered manifest are both written by this framework and nothing compared them — on taskq-done the first names `pytest_asyncio`, the tests are `pytestmark = pytest.mark.asyncio`, and requirements.txt does not name it. The rest is the two blocks' remedy text and the `UNPLACEABLE_REMEDY` import. Previous: 979.  # 2026-09-06: +38 — Round 99 站2/站4. 941 -> 979. Two changes at the same P3 site. 站4 is the wire for `unfinished_scaffolded_manifest` (+27 with the comment): `scaffold_project_manifest_from_ssot` writes a project's requirements.txt, stamps it "REVIEW AND PIN VERSIONS BEFORE COMMIT" and files a `gate:env-repair` ledger row owned by `harness`, and neither statement had a reader — 8 of the 17 corpus projects carry that row and 5 of them still ship every dependency unpinned. The checking function lives in ssot_manifest.py beside the writer, so what is here is a call, a print and a return. 站2 is +11 net at the spec-coverage block: the message stopped naming a cause it cannot know. `_run_spec_coverage_check` returns 1 from four places and only one is the threshold; this site rendered all four as "spec-coverage 0.0% < threshold 80%" plus "implement missing test cases", which on taskq-done told a run to write tests that existed while 109 declaration rows simply could not be parsed. Previous: 941.  # 2026-09-05: +941 — new entry, and it crosses the 900-line god-file threshold on the day it is granted, so the decision is reviewed here rather than drifted into. 868 -> 941: Round 98 站3. `_precheck_sab_consistency` is extracted from `_precheck_p3_security_and_quality` (which sits at its own function ceiling and is the block that had to grow), and it stops filtering `_item.actual == "not found"` — a predicate only Check 1's missing-file item ever satisfies, so Check 2's `unregistered` and Check 3's `imports X (layer Y)` were discarded 100% of the time under a headline reading "SAB architecture violations". Measured: a taskq-wow tree synthesised to produce 15 CRITICAL architecture violations put 0 of them through that filter; with the resolver fixed the twelve corpus projects produce 147. The growth is the three per-kind remediation branches (three findings, three different fixes) and the docstring recording why 57 of the 147 carry a provenance line — their source layer was chosen by amend-sab's fallback heuristic, so the line to change may be in SAD.md §2 rather than in the code. Previous: 0.
}

# 2026-09-09 — reviewed growth for the P2 transition-contract repair.  The
# 40-line transition judgement itself was kept out of both existing god files
# in cli/p2_transition.py.  The remaining growth is irreducible schema/wiring:
# exact import-contract targets (+5), typed constraint parity (+74), two
# SubAssertion provenance fields (+5), two corrected P2 auditor rows (+2), and
# the five-line advance-phase call.  Keeping these explicit here prevents the
# change from weakening the unlisted-file 900-line guard.
_LINE_CEILING.update({
    "cli/phase_cmds.py": 2280,
