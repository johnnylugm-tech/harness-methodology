...
}
{
  const aciVerify = await dispatch(
    ...
    { label: 'aci-verify', phase: 'P2 · Constitution Check — ADR', agentType: 'general-purpose', schema: GATE_VERIFY_SCHEMA },
  )
...
const testSpec = await abLoop({
  ...
  buildAPrompt: (round, prevB2) =>
    ...
    + '     6. **NFR Layering & Parameterization**: Read 01-requirements/TRACEABILITY_MATRIX.md and TEST_INVENTORY.yaml via Bash. For NFRs where TEST_SPEC.md is the verifier (Integration-level, e.g., NP-06, NP-07), you MUST define concrete `Inputs` (e.g., fault types, loop counts) and `Sub-assertions`. For all other NFRs (Unit/Static), isolate them in a `Deferred to Downstream Phases` table with columns: #, NFR, Test Function, Layer, Inputs, Title.\n'
    + '        `Inputs` is MANDATORY, same `key="value"` form as the FR tables: every threshold the AC states becomes a key — `mi_floor="80"`, `p95_budget_ms="30"`, `rows="10000"`, `skipped="0"`. A prose title alone leaves the declaration with nothing machine-checkable but its name, and P3 must READ the number it asserts rather than retype it.\n'
    ...
    + '   - **Direction B (Properties)**: If an FR has algebraic invariants (round-trip / idempotence / commutativity / invariant preservation), declare a `**Properties**` table for it: required columns `property_id | invariant | applies_to | test_function | review_ref` (+ optional `generator_strategy` / `shrinks_to`). `test_function` is exact. A `needs_review` property requires `review_ref` as `path:line`; that line must name its property ID and accepted/rejected/revised disposition before P3. The column name MUST be exactly `invariant`. Skip FRs without clean invariants.\n'
    + '     **Invariant syntax — MANDATORY**: the `invariant` cell MUST be valid Python-expression syntax (e.g. `decode(encode(source)) == source`), never prose. A symbolic variable not bound to a declared case yields `needs_review` and MUST be closed with `review_ref`; it is not silently accepted. Natural-language prose hard-blocks with `malformed_predicate`. Keep explanation outside the table.\n'
...
const aciPostSab = await dispatch(
  'Run: `' + PY + ' ' + REPO + '/harness_cli.py check-artifact-consistency --project ' + REPO + '; echo "RC=$?"`\n'
  + 'Report via StructuredOutput: verify_rc = RC= line integer; detail = the [check-artifact-consistency] / [BLOCKED] line.',
  { label: 'aci-post-sab', phase: 'P2 · Constitution Check', agentType: 'general-purpose', schema: GATE_VERIFY_SCHEMA },
)
...
for (const frId of frIds) {
  ...
  + '   AFTER RED writes the test file: open `tests/test_fr' + frNum + '.py` and ensure EVERY NFR associated with ' + frId + ' in the traceability table (TRACEABILITY_MATRIX.md §5 is the canonical listing) has a `# NFR-XX` comment on at least one test function. Without these annotations, `compute_trace_dimension` 4c = 0% and Gate 2 blocks (HR-16). Use `grep -n "# NFR-" tests/test_fr' + frNum + '.py` and check against the NFR list for ' + frId + ' — document every association.\n'
      ...
      + '   c. `' + PY + ' ' + REPO + '/harness_cli.py review-fr-tests --fr-id ' + frId + ' --phase 3 --project ' + REPO + '` — asks one reviewer which assertion of ' + frId + ' fails if SPEC.md\'s requirement is violated. REJECT → fix the TEST so that one does (not the implementation, and not by weakening the assertion), then repeat step 6 and this step. Max 2 rounds; still REJECT → report it and move on, advance-phase blocks on it at the P3 exit.\n'
...
phase('P3 · Gate 2')
log('Gate 2 exit (composite ≥75, 12 dims: 9 self-scored + mutation_testing/traceability/architecture framework-owned)')
...
for (const frId of deltaTodo) {
  ...
  const frReport = await dispatch(
    ...
    { label: 'delta-' + frId, phase: 'P4 · Per-FR Delta', agentType: 'general-purpose', schema: FR_STEP_SCHEMA },
  )
  if (frReport === null || frReport === undefined || typeof frReport !== 'object') {
    log('  ' + frId + ' agent blocked (session limit / rate limit) — aborting retries, resume after quota reset')
...
for (const frId of deltaTodo) {
  ...
  const frReport = await dispatch(
    ...
    + 'Report via the StructuredOutput tool: { rc: <the integer from step 1c\'s last RC= line>, final_line: "' + frId + ' GATE1: PASS" or "' + frId + ' GATE1: FAIL — <reason>" }.\n\n'
    + 'SCOPE RULES:\n- DO NOT touch any FR OTHER than ' + frId + '.\n- DO NOT run advance-phase / push-milestone / generate BASELINE docs.\n- DO NOT edit .methodology/quality_manifest.json or .sessi-work/gate1_result.json to fake/reset scores — fix the underlying code/tests instead.\n- DO NOT modify harness/.\n- ONLY GATE1-DELTA (+ full TDD if needed) for ' + frId + '.',
    { label: 'delta-' + frId, phase: 'P5 · Per-FR Delta', agentType: 'general-purpose', schema: FR_STEP_SCHEMA },
  )
  if (frReport === null || frReport === undefined || typeof frReport !== 'object') {
    log('  ' + frId + ' agent blocked (session limit / rate limit) — aborting retries, resume after quota reset')
  ...
  if (frRc === 70) {
    log('  ' + frId + ' exited 70 — harness-methodology crashed, aborting remaining FRs')
    return { harness_bug_detected: true, phase: 5, fr_id: frId, gate1Pass, gate1Fail: [...gate1Fail, frId], message: frId + ' GATE1-DELTA: harness-methodology itself crashed (exit 70 — see the crash bundle path in the log). This is not a project quality issue; a human must diagnose and fix the harness bug before this FR can proceed.' }
  }
  if (frRc === 25) {
    log('  ' + frId + ' exited 25 — INFRA precondition block, aborting remaining FRs')
...
phase('P6 · Gate 4')
log('Gate 4 full-project eval (composite ≥85, 16 dims: 13 self-scored + mutation_testing/architecture/traceability framework-owned)')
...
if (!gate4Pass) {
  log('  Gate 4 exhausted 3 rounds — generating deferred_fixes.md')
  const gate4StateCmd = PY + ' -c "import json; g=(json.load(open(\'' + REPO + '/.methodology/quality_manifest.json\')).get(\'gate_results\',{}) or {}).get(\'gate4\') or {}; print(json.dumps({\'score\': g.get(\'score\'), \'qc\': g.get(\'quality_complete\'), \'dims\': g.get(\'dimensions\',{})}))"'
  await dispatch(
    'YOU ARE THE DEFERRED-FIX RECORDER. Gate 4 failed to reach PASS in 3 rounds.\n'
    + 'REPO: ' + REPO + '\nPYTHON: ' + PY + '\n\n'
    + '1. Get the last-known Gate 4 state:\n`' + gate4StateCmd + '`\n'
    + '2. Run `' + PY + ' ' + REPO + '/harness_cli.py spec-coverage-check --project ' + REPO + ' --threshold 90.0; echo "RC=$?"` for the D4 status.\n'
    + '3. Run `' + PY + ' ' + REPO + '/harness_cli.py crg-arch-check --project ' + REPO + '; echo "RC=$?"` for the CRG architecture status.\n'
    + '4. Write `' + REPO + '/.methodology/deferred_fixes.md` with:\n'
    + '   - A brief header: "Gate 4 — deferred fixes" + date + last-known composite score\n'
    + '   - Each failing dimension (score below its threshold) as a `- [ ]` checkbox item\n'
    + '   - D4 as a `- [ ]` checkbox item (spec-coverage < 80%)\n'
    + '   - CRG architecture as a `- [ ]` checkbox item if RC != 0 (architecture score < 80%)\n'
    + '   - Each item MUST cite the current score AND the required threshold\n'
...
for (const frId of deltaTodo) {
  ...
  const frReport = await dispatch(
    ...
    + 'Steps:\n'
    + '1. GATE1-DELTA — long-running when code changed (every internal fix round spawns a fixer AND re-dispatches a full GATE1, so the wall time is the budget the cap in step b encodes). Run it BACKGROUNDED, do NOT invoke it as a plain synchronous command:\n'
    + '   a. `nohup bash -c \'' + PY + ' ' + REPO + '/harness_cli.py run-fr-step --phase 7 --fr-id ' + frId + ' --step GATE1-DELTA --project ' + REPO + '; echo "RC=$?"\' > /tmp/gate1delta_' + frId + '.log 2>&1 & echo $!` — note the PID.\n'
    ...
    + 'Report via the StructuredOutput tool: { rc: <the integer from step 1c\'s last RC= line>, final_line: "' + frId + ' GATE1: PASS" or "' + frId + ' GATE1: FAIL — <reason>" }.\n\n'
    + 'SCOPE RULES:\n- DO NOT touch any FR OTHER than ' + frId + '.\n- DO NOT run advance-phase / push-milestone / generate risk docs.\n- DO NOT edit .methodology/quality_manifest.json or .sessi-work/gate1_result.json to fake/reset scores — fix the underlying code/tests instead.\n- DO NOT modify harness/.\n- ONLY GATE1-DELTA (+ full TDD if needed) for ' + frId + '.',
    { label: 'delta-' + frId, phase: 'P7 · Per-FR Delta', agentType: 'general-purpose', schema: FR_STEP_SCHEMA },
  )
  if (frReport === null || frReport === undefined || typeof frReport !== 'object') {
    log('  ' + frId + ' agent blocked (session limit / rate limit) — aborting retries, resume after quota reset')
  ...
  if (frRc === 70) {
    log('  ' + frId + ' exited 70 — harness-methodology crashed, aborting remaining FRs')
    return { harness_bug_detected: true, phase: 7, fr_id: frId, gate1Pass, gate1Fail: [...gate1Fail, frId], message: frId + ' GATE1-DELTA: harness-methodology itself crashed (exit 70 — see the crash bundle path in the log). This is not a project quality issue; a human must diagnose and fix the harness bug before this FR can proceed.' }
  }
  if (frRc === 25) {
    log('  ' + frId + ' exited 25 — INFRA precondition block, aborting remaining FRs')
  ...
  if (verdict === null || verdict === undefined || typeof verdict !== 'object') {
    log('  ' + frId + ' agent blocked (session limit / rate limit) — aborting retries, resume after quota reset')
    return { session_limit_blocked: true, phase: 7, step: frId, fr_id: frId, gate1Pass, message: 'Agent hit session/rate limit verifying ' + frId + ' Gate 1. Resume after quota reset — the manifest read is idempotent.' }
  }
  const passed = String((verdict && verdict.reason) || '').trim().startsWith('GATE1_VERIFIED_PASS')
  if (!passed && frRc === -1) {
    log('  ' + frId + ' — GATE1-DELTA killed at the poll cap; no manifest verdict')
    return { fr_step_timeout: true, halt_step: 'fr-step-timeout', phase: 7, fr_id: frId, gate1Pass, gate1Fail, message: frId + ' GATE1-DELTA: killed at the poll cap with run-fr-step still running, so no gate verdict was reached — this is NOT a code-quality failure and no fix agent should be sent at it. Re-run with a NEW run_tag: Workflow({scriptPath, args: {repo, run_tag}}); a recurrence means the step is hung past the budget computed from fr_step timeout and max_fix_rounds.' }
  }
  if (passed) {
    gate1Pass.push(frId); log('  ' + frId + ' Gate 1 PASS [harness-verified]')
  } else { gate1Fail.push(frId); log('  ' + frId + ' Gate 1 FAIL [harness manifest qc != true; sub-agent self-report ignored]') }
...
if (gate1Fail.length) {
  return halt('gate1', { error: 'Phase 7: Gate 1 FAILED for FR(s): ' + gate1Fail.join(', ') + ' (escalate)', owner: 'project', gate1Pass, gate1Fail })
}
if (gate1Pass.length) {
  await dispatch(
    'Run these commands via the Bash tool, in order. Report the verbatim stdout/stderr of ALL of them.\n'
...
for (const frId of deltaTodo) {
  ...
  const frReport = await dispatch(
    ...
    + 'Report via the StructuredOutput tool: { rc: <the integer from step 1c\'s last RC= line>, final_line: "' + frId + ' GATE1: PASS" or "' + frId + ' GATE1: FAIL — <reason>" }.\n\n'
    + 'SCOPE RULES:\n- DO NOT touch any FR OTHER than ' + frId + '.\n- DO NOT run push-milestone / generate config docs / create archive.\n- DO NOT edit .methodology/quality_manifest.json or .sessi-work/gate1_result.json to fake/reset scores — fix the underlying code/tests instead.\n- DO NOT modify harness/.\n- ONLY GATE1-DELTA (+ full TDD if needed) for ' + frId + '.',
    { label: 'delta-' + frId, phase: 'P8 · Per-FR Delta', agentType: 'general-purpose', schema: FR_STEP_SCHEMA },
  )
  if (frReport === null || frReport === undefined || typeof frReport !== 'object') {
    log('  ' + frId + ' agent blocked (session limit / rate limit) — aborting retries, resume after quota reset')
  ...
  if (verdict === null || verdict === undefined || typeof verdict !== 'object') {
    log('  ' + frId + ' agent blocked (session limit / rate limit) — aborting retries, resume after quota reset')
    return { session_limit_blocked: true, phase: 8, step: frId, fr_id: frId, gate1Pass, message: 'Agent hit session/rate limit verifying ' + frId + ' Gate 1. Resume after quota reset — the manifest read is idempotent.' }
  }
  const passed = String((verdict && verdict.reason) || '').trim().startsWith('GATE1_VERIFIED_PASS')
  if (!passed && frRc === -1) {
    log('  ' + frId + ' — GATE1-DELTA killed at the poll cap; no manifest verdict')
    return { fr_step_timeout: true, halt_step: 'fr-step-timeout', phase: 8, fr_id: frId, gate1Pass, gate1Fail, message: frId + ' GATE1-DELTA: killed at the poll cap with run-fr-step still running, so no gate verdict was reached — this is NOT a code-quality failure and no fix agent should be sent at it. Re-run with a NEW run_tag: Workflow({scriptPath, args: {repo, run_tag}}); a recurrence means the step is hung past the budget computed from fr_step timeout and max_fix_rounds.' }
  }
  if (passed) {
    gate1Pass.push(frId); log('  ' + frId + ' Gate 1 PASS [harness-verified]')
  } else { gate1Fail.push(frId); log('  ' + frId + ' Gate 1 FAIL [harness manifest qc != true; sub-agent self-report ignored]') }
...
if (gate1Fail.length) {
  return halt('gate1', { error: 'Phase 8: Gate 1 FAILED for FR(s): ' + gate1Fail.join(', ') + ' (escalate)', owner: 'project', gate1Pass, gate1Fail })
}
if (gate1Pass.length) {
  await dispatch(
    'Run these commands via the Bash tool, in order. Report the verbatim stdout/stderr of ALL of them.\n'
