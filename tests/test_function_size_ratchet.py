...
_CEILINGS: dict[str, int] = {
    ...
    "cli/fr_cmds.py::cmd_run_fr_step": 872,  # 2026-09-08: 856 -> 872 — Round 109 站6: the undeclared-floor branch in COVERAGE-FIX (a record_degradation naming `quality_targets.min_coverage`, four lines of comment, and one line added to the guarded comparison).  # 2026-09-07: 810 -> 856 — Round 102 站2: canonical budget-kill / blocked-human failure rows in the escalation recorders and both blocked exits, plus the fix-round pre-dispatch repeated_failure refusal. Previous: 810.
    "scripts/workflowgen/spec_phase3.py::_render_per_fr_tdd": 237,  # 2026-09-07: new — Round 102 站3: gate1Parked collector, parked-continue in the per-FR tail, and the post-loop re-attempt block with the gate1-parked halt (parked FRs re-run GATE1 only). Previous: 0.  # 2026-09-06: 798 -> 810 — fix(fr-step-no-progress): wire diff_sig and block_sig into the lateral variation detector so file content changes and block item reductions register as progress.  # 2026-09-05: 770 -> 798 — Round 96 站0. The SUITE_TEST_FAILURE branch: a red whole-suite run now routes to TEST-FIX with the failing nodeids and the command that reproduces them, instead of to COVERAGE-FIX. 28 lines in the dispatch chain beside the eight branches already there. Measured cost of not having it: FR-07 on taskq-final Phase 8, 22 rounds and 9.5 hours, 67 of that run's 620 dispatches.
