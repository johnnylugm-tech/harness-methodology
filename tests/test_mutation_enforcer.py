...
# Bug: bare except swallows sqlite errors, making them indistinguishable from zero mutants
# --------------------------------------------------------------------------------------
...
def test_sqlite_error_not_swallowed_as_zero_mutants(tmp_path, monkeypatch):
    ...
    # Create the cache directory that _count_mutmut_results will try to open.
    # Content doesn't matter since sqlite3.connect is patched to raise.
    cache_dir = fake_workdir / ".mutmut-cache"
    cache_dir.mkdir()
    (cache_dir / "mutmut.db").write_text("dummy", encoding="utf-8")
