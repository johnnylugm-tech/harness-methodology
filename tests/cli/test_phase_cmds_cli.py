...
class TestAdvancePrechecksSubtraction:
    """Round 25 站3 — two things _advance_prechecks no longer does.

    Both were WARN-only: they enforced nothing, and both printed into the one
    output stream workflow JS instructs the agent to read verbatim and act on.
    Noise there is not free — it competes with the [BLOCKED] lines that ARE
    instructions.
    """

    def test_no_network_call_for_submodule_drift(self):
        """It was advance-phase's ONLY network call — a `git fetch` on every
        phase transition, 60% of the wall time of a P1 or P2 advance, blocking
        nothing. It now runs on demand in `doctor`
        (core.doctor._check_submodule_behind), whose relocated behaviour tests
        live in tests/test_doctor.py::TestSubmoduleBehindOrigin.
        """
        import inspect
        from cli.phase_cmds import _advance_prechecks

        src = inspect.getsource(_advance_prechecks)
        assert "_check_submodule_drift" not in src
        assert "behind_count" not in src

    def test_no_hardcoded_python_web_stack_advisory(self):
        """The fastapi/httpx advisory fired unconditionally for every P3+
        advance regardless of what the project is or what language it is
        written in, and enforced nothing. On the run-all-by-workflow run it
        told a CLI job-queue tool to `pip install fastapi` six times.
        """
        import inspect
        from cli.phase_cmds import _advance_prechecks

        src = inspect.getsource(_advance_prechecks)
        for pkg in ("fastapi", "httpx"):
            assert f'"{pkg}"' not in src and f"'{pkg}'" not in src, (
                f"the {pkg} advisory is back — a guess about the next phase's "
