...
def build_tdd_red_prompt(fr_id: str, phase: int, project: Path, srs_path: Path, test_file: str, src_dir: str) -> str:
    ...
    return (
        ...
        f"  THIS LICENCE IS ABOUT SCENARIOS, NEVER ABOUT THRESHOLDS. When the AC states\n"
        f"  a number (MI >= 80, p95 < 30ms, coverage >= 80, zero skips), that number is\n"
        f"  the criterion — asserting a weaker one is not a simpler test of the same\n"
        f"  thing, it is a test of a different thing that will pass while the criterion\n"
        f"  fails. If you cannot make the declared threshold hold, leave the test RED\n"
        f"  and say so; do not lower it.\n"
