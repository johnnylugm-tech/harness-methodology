...
def _precheck_manifest_and_p1_baselines(_mi, completed_phase, project) -> "int | None":
    ...
    # ── Every later exit: has the declaration moved since P1? ─────────
    # Round 111 站F1. The checksum above was written for eight versions
    # with nothing reading it back, and the check it protects
    # (spec_coverage's P1 Naming Authority) reads the working-tree file —
    # so retracting a declared name satisfies it. 15 of the 21 corpus
    # projects carry a digest that no longer matches their file.
    # Recorded, not blocked: taskq-new's 50 "retracted" names include one
    # split into the eight TEST_SPEC.md rows it always meant, and charging
    # the only project that refined its declaration is Round 46.
    # Here rather than in spec_coverage because that function runs several
    # times per phase and this must be one row per advance.
    if completed_phase >= 2:
        ...
        if _moved:
            ...
            record_degradation(
                project, "spec-coverage:naming-authority",
                f"TEST_INVENTORY.yaml declares {len(_moved['retracted'])} fewer "
                f"and {len(_moved['added'])} more test names than at the P1 exit",
                why=("the P1 Naming Authority check reads the working-tree file, "
                     "so a retracted declaration passes it silently"),
                data=_moved, owner="project",
            )
    # Generated by the extraction, not moved with the run: mypy
    # requires the fall-through path to be explicit.
    return None
