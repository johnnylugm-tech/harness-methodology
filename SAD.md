...
| `spec_shared.py` | The one renderer grep-confirmed shared across all 8 phases (`_render_meta`) — see Round 15 station1 for the call-count verification method |
| `spec_phase1.py` .. `spec_phase8.py` | One `generate_phaseN()` per phase (Round 15 split from the former monolithic `phase_specs.py`) — declarative step sequences, milestone type, artifacts allowlist; genuine per-phase differences (e.g. phase1's `runSubTask` vs phase2's `abLoop` — confirmed non-unifiable by full diff, see module comments) are kept as phase-local verbatim blocks rather than forced into a shared parameter surface. `spec_phase1.py` is the largest (4 serial A/B sub-tasks + Load Legal Artifacts + Forward Ref Check + a dedicated `runPeerReview`) and the only one over the file-size ratchet threshold |
| `phase_specs.py` | Facade only (Round 15) — re-exports `generate_phase1`..`generate_phase8` from the `spec_phaseN` modules so `generate_workflows.py` and existing tests import unchanged |
