...
const envReport = await agent(
  ...
  + PY + ' ' + REPO + '/harness_cli.py run-env-check --phase 5 --project ' + REPO + ' && ' + PY + ' ' + REPO + '/harness_cli.py finalize-env-check --phase 5 --project ' + REPO + '; echo "RC=$?"\n'
...
if (!(envReport && envReport.rc === 0)) {
  return { error: 'Phase 5 env-check did not PASS', rc: envReport ? envReport.rc : null, note: envReport ? 'run-env-check/finalize-env-check exit ' + envReport.rc + ' — read .sessi-work/env_check_result.json' : 'agent returned null (skipped or terminal API error)' }
}
...
const integrityCmd = PY + ' ' + REPO + '/harness_cli.py check-manifest-integrity --project ' + REPO + ' --phase 5'
