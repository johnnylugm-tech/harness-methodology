...
for (const frId of frIds) {
  ...
  + '   AFTER RED writes the test file: open `tests/test_fr' + frNum + '.py` and ensure EVERY NFR associated with ' + frId + ' in the traceability table (TRACEABILITY_MATRIX.md §5 is the canonical listing) has a `# NFR-XX` comment on at least one test function. Without these annotations, `compute_trace_dimension` 4c = 0% and Gate 2 blocks (HR-16). Use `grep -n "# NFR-" tests/test_fr' + frNum + '.py` and check against the NFR list for ' + frId + ' — document every association.\n'
      ...
      + 'Implement the module per SPEC.md (read ' + REPO + '/SPEC.md for ' + frId + ') + SAD.md module mapping. Write source under `03-development/src/<module>/` (the harness-scaffolded canonical layout — see `init-project`\'s directory scaffold). Do NOT place this FR\'s implementation inside a file another FR already owns or a shared/global file (e.g. `cli.py`) used by multiple FRs — each FR\'s logic belongs in its own module directory per SAD.md/quality_manifest.json\'s fr_module_traceability mapping. Tests under `tests/` per project layout (MIRROR step above already validated them at that path). All tests for ' + frId + ' MUST live in the single canonical file `tests/test_fr' + frNum + '.py` — this is the only filename the harness coverage/RED-check/GATE1-DELTA diff tooling recognizes. Do not create satellite files like `test_fr' + frNum + '_unit.py`; add more test functions to the one file instead — this single-file rule applies ONLY to tests, NOT to source (do not cross-apply it to `03-development/src/`). Docstrings must include [' + frId + '] reference (NFR-05).\n\n'
